...
RECEIVERS = ["2387993145@qq.com"]  # 你的QQ邮箱
LOCATION_ID = "101270101" # 成都

# 🌟 你的专属 API Host (这是关键修改！)
...
def get_weather_data():
    ...
    try:
        ...
        if 'code' not in resp_now or resp_now['code'] != '200':
            print(f"❌ 实时天气请求失败，返回内容: {resp_now}")
            return None
            
        # 2. 获取预报
        resp_daily = requests.get(url_daily).json()
        if 'code' not in resp_daily or resp_daily['code'] != '200':
            print(f"❌ 天气预报请求失败，返回内容: {resp_daily}")
            return None
            
        # 3. 获取指数
        resp_indices = requests.get(url_indices).json()
        if 'code' not in resp_indices or resp_indices['code'] != '200':
            print(f"❌ 生活指数请求失败，返回内容: {resp_indices}")
            return None
